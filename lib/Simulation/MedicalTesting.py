...
class MedicalTesting:
    ...
    def test(self,agent,timeStamp):        
        if (self.rng.uniform(0.0,1.0) < self.accuracy):
            # correct result
            agent.testResult = PCRResult(agent, (agent.infection is not None) ,timeStamp,3*3600) 
        else:
            # incorrect result
            agent.testResult = PCRResult(agent, not (agent.infection is not None) ,timeStamp,3*3600) 
